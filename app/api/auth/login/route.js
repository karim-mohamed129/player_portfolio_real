...
export async function POST(request) {
  ...
  try {
    ...
    if (rawText.length > 12_000) return NextResponse.json({ error: "حجم الطلب كبير جداً." }, { status: 413 });
    ...
    if (!challenge.ok) {
      await sleep(450);
      return NextResponse.json({ error: challenge.error || "تعذر إكمال التحقق الأمني. حاول مرة أخرى." }, { status: 400 });
    }
    ...
    return safeErrorResponse(error, "تعذر تسجيل الدخول. تأكد من البيانات وحاول مرة أخرى..", 500);
  }
}
