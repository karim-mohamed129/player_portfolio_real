...
export function adminIpDeniedResponse() {
  return NextResponse.json({ error: "Admin access is restricted from this network." }, { status: 403 });
}
...
export function safeErrorResponse(error, fallbackMessage = "Internal server error.", status = 500) {
...
export function requireAdminAjaxHeader(request) {
  ...
  if (header.toLowerCase() !== "xmlhttprequest") {
    return NextResponse.json({ error: "Missing secure request header." }, { status: 403 });
  }
  return null;
}
...
export function isSafeCloudinaryPdfUrl(value) {
  ...
  try {
    const url = new URL(raw);
    return url.protocol === "https:" && url.hostname === "res.cloudinary.com" && url.pathname.toLowerCase().endsWith(".pdf");
  } catch {
    return false;
  }
}
